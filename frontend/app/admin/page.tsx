...
export default function AdminPage() {
  ...
  const fetchEvents = async (email?: string) => {
    ...
    try {
      const res = await fetch(
        `http://localhost:3001/events?creatorEmail=${email}`
      );
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        await fetchEvents(currentUser.email);
        resetForm();
      } else {
        setMessage(`❌ ${data.message}`);
      }
  ...
  const handleDelete = async (id: number) => {
    ...
    const res = await fetch(
      `http://localhost:3001/events/${id}?email=${currentUser.email}`,
      { method: 'DELETE' }
    );
          ...
          <div className="lg:col-span-5">
            <div className="bg-white p-6 rounded-xl shadow-lg sticky top-8">
              ...
              <form onSubmit={handleSubmit} className="space-y-3">
                ...
                {message && (
                  <div className="p-3 bg-slate-100 rounded text-center text-sm font-bold animate-pulse">
                    {message}
                  </div>
                )}
              </form>
            ...
            <div className="bg-white p-6 rounded-xl shadow-lg">
              ...
              <div className="space-y-3 max-h-[600px] overflow-y-auto">
                {events.map((ev) => (
                  <div
                    key={ev.id}
                    className="flex flex-col sm:flex-row justify-between items-start sm:items-center p-4 border border-indigo-100 bg-indigo-50/30 rounded-lg hover:shadow-md transition-all"
                  >
                    ...
                    <div className="flex gap-2 w-full sm:w-auto">
                      <button
                        ...
                        className="flex-1 sm:flex-none bg-white border border-indigo-200 text-indigo-600 px-3 py-1.5 rounded-lg text-sm font-bold hover:bg-indigo-50 transition-colors"
                      >
                        Düzenle
                      </button>
                      <button
                        onClick={() => handleDelete(ev.id)}
                        className="flex-1 sm:flex-none bg-white border border-red-200 text-red-600 px-3 py-1.5 rounded-lg text-sm font-bold hover:bg-red-50 transition-colors"
                      >
                        Sil
                      </button>
                    </div>
